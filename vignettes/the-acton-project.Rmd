...
Transport was the UK's largest and fastest growing source of carbon emissions before the Covid-19 pandemic [@departmentfortransport_decarbonising_2020].
The government's aggressive and legally binding climate targets, plus overwhelming evidence of the health and social benefits of active travel, make it vital that new developments are conducive to low-carbon lifestyles.
This means low levels of car ownership and use, and high levels of walking and cycling for everyday trips.
Unfortunately recent trends have been in the opposite direction [@hickman_looking_2007] and it is possible that unsustainable 'business as usual' transport systems become embedded again after social distancing measures are relaxed.
New residential developments have tended to be located in places that lock residents into car dependent lifestyles, something that central government aims to tackle [@mhclg_planning_2020].
...
Several outputs that were developed during the course of the project.
These, and the potential for further work to inform regional and national development policies, were discussed in a workshop held in Leeds in March 2020. 

On the technical side, we developed open source software to support the project, an R package named `acton`, to ensure reproducibility and to provide a strong foundation for further work.
The package contains functions that aid the import of planning application and accessibility data.

We tested the methods on a case study of Leeds to investigates the accessibility for active travel of a set of new housing developments in Leeds, West Yorkshire.
...
These updates are were based on a development version of PlanIt API that will be incorporated into the [main PlanIt website](https://www.planit.org.uk/) by end of May 2020.
...
The `acton` R package (described below) includes a function for the import of the journey time statistics into R.
...
Available fields include links to the full PlanIt record and to planning application information from Leeds City Council, as illustrated in
...
The travel times in these statistics are recorded from the population weighted centroid of each LSOA. New housing developments can be constructed anywhere within an LSOA, and may not necessarily be close to existing homes. To assess the accessibility of new developments we therefore use statistics that relate to the closest LSOA centroid. For our Tyersal Lane case study site, the closest LSOA centroid is in Bradford.

Multiple statistics are available for access to centres of employment, including travel time to locations with 100-499 jobs, to locations with 500-4999 jobs, and to locations with 5000+ jobs. We combined these three statistics to produce a single employment metric weighted by the number of jobs.
This is shown in maps below.
...
According to these measures, there are many areas that have poor accessibility by active modes and public transport. We can see that three of the four case study sites are located in areas that have relatively poor accessibility to local services. Tyserall, Micklefield and Allerton Bywater are all in areas where, according to our combined index, local services are >15 minutes distant on average by walking or public transport. Micklefield is in an area where local services are >20 minutes distant by cycling.

## Travel to work and route busyness

Journeys to work by case study site residents are estimated based on OD data for commutes for the LSOA which has its population weighted centroid closest to each site. These journeys are allocated to the transport network using CycleStreets.net routing algorithms.
The point of origin was updated so rather than starting at the nearest LSOA centroid, the routes start within the case study sites themselves. These desire lines were then converted to routes using 'fastest journey' option of the `cyclestreets::journey()` function and the routing capabilities in the R package **stplanr** [@lovelace_stplanr_2018]. 
...
The thickness of the lines in the map below, which illustrates route busyness data, is proportional to the number of journeys (using all modes of transport), and the colour represents the busyness of the roads. 
For other maps, see the Case Studies report on the ACTON website.
...
The final component of the ACTON project is a prototype indicator for active travel levels in new homes.
...
The range of explanatory variables can be expanded further as more data becomes available, adding factors such as the circuity of the routes. 

For the prototype version of this model we have used data from 12 sites across England, including Allerton Bywater in Leeds. The number of sites will expand as we further develop the model. 
...
We use the `glm()` R package, with a quasibinomial probability distribution and a logit link function, since the proportion of commuters who cycle can be represented as the result of a series of binomial trials.

Having investigated various candidate explanatory variables for distance and hilliness, the following model was found to have the lowest deviance.
