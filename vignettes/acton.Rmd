...
The transport sector is now the UK's largest and --- before the coronavirus pandemic --- fastest growing source of greenhouse gas emissions.
The construction sector and transport planning policies can contribute to the solutions, which include fostering community cohesion to increase the strength of social connections locally and thereby reducing the need for travel, enabling walking and cycling, discouraging high levels of car ownership and encouraging use of public and shared mobility.
Unfortunately, things have been moving in the opposite direction recently, with new residential developments tending to be located and designed in ways that promote car dependency.
This risks locking people into unsustainable lifestyles in future and endangers efforts to hit stringent emissions targets (zero carbon by 2050 in the UK).
The ACTON project, which stands for active transport opportunities for new developments, brings together several key datasets to answer the question how can we modify existing developments, and improve the location and design of new housing developments, to ensure high levels of walking and cycling? 
The main stages of this three month project, funded by [Research England](https://re.ukri.org/), were to: 1) provide evidence on the current level of provision and around new developments; 2) develop a methodology to assess the potential for cycling and walking; and 3) identify options that would improve provision, increasing walking and cycling levels cost-effectively.
To do this we first created strong and scalable techniques that enable automated access to key data sources nationwide.

To ensure reproducibility and transparency of results, these techniques were encoded in an open source R package called **acton**.
The **acton** package provides access to four main types of data: 1) planning data from the new PlanIt API; 2) journey time data from the Department for Transport; 3) travel and demographic data from the Census (and other sources in future); and 4) vital route information such as the level of busyness and circuity (how far from the crow flies) on key walking and cycling desire lines.
This is the first time that these four types of data have been made available in a single project and our analysis demonstrates the feasibility of using objective national data as the basis for decision-making relevant to new developments.
Attendees of the ACTON workshop including central and local government (with representatives from the Department for Transport and local authorities), consultancies (with three transport planning consultancies sending delegates) and active travel advocates (including the Policy Director of CyclingUK) provided positive feedback on our work, which has been written-up and documented on the publicly accessible website at [cyipt.github.io](https://cyipt.github.io/acton/).
...
We plan to take the work forward after this positive feedback and a follow-on meeting with the Department for Transport and the Ministry of Housing, Communities and Local Government.
We plan pursue funding bids that will put the methods into production, to create a national indicator and web tool to support local authorities and other stakeholders tasked with ensuring that future developments have good active transport provision supporting wider social, health and environmental policy objectives.
